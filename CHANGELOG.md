...
### Added
- One-line bash installation script `install.sh` for Linux & macOS (Closes #2).
- One-line PowerShell installation script `install.ps1` for Windows (Closes #2, #3).
- Enhanced Windows WMI & PowerShell CPU/GPU/RAM detection (Closes #3).
- Enhanced macOS `sysctl` and `system_profiler` hardware detection routines (Closes #3).
